...
imp = IJ.openImage(pathpath+'/BBB_MaskMerge.tif') 
...
fm = model.getFeatureModel()
outpath = (pathpath+'/BBB_tracks.csv')
with open(outpath,'w') as resultFile:
    csvWriter = csv.writer(resultFile, delimiter=',', lineterminator='\n')
    csvWriter.writerow(['TRACK_ID','POSITION_X','POSITION_Y','FRAME','MEAN_INTENSITY_CH1','MEDIAN_INTENSITY_CH1','MAX_INTENSITY_CH1','MIN_INTENSITY_CH1','SUM_INTENSITY_CH1','AREA'])
# Iterate over all the tracks that are visible.
    for id in model.getTrackModel().trackIDs(True):
        ...
        for spot in track:
            ...
            sumin = spot.getFeature('SUM_INTENSITY_CH1')
            area1 = spot.getFeature('AREA')
            model.getLogger().log('\tspot ID = ' + str(sid) + ': x='+str(x)+', y='+str(y)+', t='+str(t)+', q='+str(q) + ', snr='+str(snr) + ', mean = ' + str(mean))
            csvWriter.writerow([str(id+1),str(x),str(y),str(t),str(mean),str(median),str(maxin),str(minin),str(sumin),str(area1)])
